    ...
    return {
        async find(query, data) {
            ...
            if (!query) {
                ...
                res = await s.db.query('SELECT * FROM ' + options.tableName + ' WHERE ' + query, data)
            }
        ...
        async findOne(query, data) {
            let res
            if (!query) {
                res = await s.db.query('SELECT * FROM ' + options.tableName + ' LIMIT 1')
            }
            else {
                res = await s.db.query('SELECT * FROM ' + options.tableName + ' WHERE ' + query + ' LIMIT 1', data)
            }
        ...
        async save(obj, silent = false) {
            ...
            if (options.emptyRow) {
                if (options.emptyRow.created_date !== undefined) {
                    if (!obj.created_date) {
                        obj.created_date = new Date().toISOString().slice(0, 19).replace('T', ' ')
                    }
                }

                if (options.emptyRow.updated_date !== undefined) {
                    if (!obj.updated_date) {
                        obj.updated_date = new Date().toISOString().slice(0, 19).replace('T', ' ')
                    }
            ...
            if (keys.indexOf('id') === -1  || obj.id === null) {
                query = 'INSERT INTO ' + options.tableName + '(' + keys.join(', ') + ') VALUES(' + '?, '.repeat(keys.length - 1) + '?)'
            }
            else {
